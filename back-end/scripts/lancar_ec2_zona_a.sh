...
security_group_id=$(aws ec2 describe-security-groups --group-names "novoTrello-dev" --query "SecurityGroups[0].GroupId" --output text 2>/dev/null)

if [ -z "$security_group_id" ]; then
    echo ">[ERRO] Security group novoTrello-dev não foi criado na VPC $vpc_id"
    exit 1
fi
